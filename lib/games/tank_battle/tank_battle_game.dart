...
class TankBattleGame extends FlameGame with MultiTouchTapDetector {
  ...
  Rect _getPlayerZone(int index) {
    switch (players.length) {
      ...
      case 3:
        if (index == 0) return Rect.fromLTWH(0, size.y / 2, size.x / 2, size.y / 2);
        if (index == 1) return Rect.fromLTWH(size.x / 2, 0, size.x / 2, size.y / 2);
        return Rect.fromLTWH(0, 0, size.x / 2, size.y / 2);
      case 4:
        final x = (index % 2) * size.x / 2;
        final y = (index < 2) ? size.y / 2 : 0.0;
